...
void Player::update(){
    ...
    if(TheInputHandler::Instance()->isKeyDown(SDL_SCANCODE_D)||TheInputHandler::Instance()->isKeyDown(SDL_SCANCODE_A)||TheInputHandler::Instance()->isKeyDown(SDL_SCANCODE_S)||TheInputHandler::Instance()->isKeyDown(SDL_SCANCODE_W)){
        ...
        if(TheInputHandler::Instance()->isKeyDown(SDL_SCANCODE_D)){
            if(m_position.getX() >= 820) m_velocity.setX(0);
            else{
        ...
        if(TheInputHandler::Instance()->isKeyDown(SDL_SCANCODE_A)){
            if(m_position.getX() <= 0) m_velocity.setX(0);
            else{
            m_currentFrame = int(((SDL_GetTicks() / 100) % 7));
            m_velocity.setX(-3);
            m_velocity.setY(0);
        ...
        if(TheInputHandler::Instance()->isKeyDown(SDL_SCANCODE_W)){
            if(m_position.getY() <= 0) m_velocity.setY(0);
            else{
                m_currentFrame = int(((SDL_GetTicks() / 100) % 7));
                m_velocity.setX(0);
                m_velocity.setY(-3);
        ...
        if(TheInputHandler::Instance()->isKeyDown(SDL_SCANCODE_S)){
            if(m_position.getY() >= 440) m_velocity.setY(0);
            else{
                m_currentFrame = int(((SDL_GetTicks() / 100) % 7));
                m_velocity.setX(0);
                m_velocity.setY(3);
